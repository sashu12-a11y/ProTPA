...
public class TpahereCommand implements CommandExecutor {
    ...
    public TpahereCommand(ProTPA plugin) {
        this.plugin = plugin;
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length < 1) {
            player.sendMessage(messageManager.getMessage("usage-tpa").replace("{command}", "tpahere <player>"));
            return true;
        }
        ...
        RequestData request = new RequestData(player.getUniqueId(), RequestData.RequestType.TPAHERE);
